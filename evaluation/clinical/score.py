...
required_test_df_cols = list(set(parse_spec(args.metric_src_field_specs)+parse_spec(args.metric_tgt_field_specs)+parse_spec(args.dataset_id_column_specs)))

test_df = test_df[required_test_df_cols]
...
def compute_summary_metric(row):
    ...
    # row['questeval_w_ref'] = questeval_scorer(src_txts, gen_txts, [tgt_txts])['score_details_per_sample']['questeval'][0]
    # row['questeval_no_ref'] = questeval_scorer(src_txts, gen_txts)['score_details_per_sample']['questeval'][0]

    # summac
    summac_scorer = SummaCMetric()

    summac_dict_ref = summac_scorer(src_txts, tgt_txts)
    summac_dict_gen = summac_scorer(src_txts, gen_txts)

    row['summac_zs_ref'] = summac_dict_ref['score_details_per_sample']['SummaC-ZS'][0]
    row['summac_zs_gen'] = summac_dict_gen['score_details_per_sample']['SummaC-ZS'][0]

    row['summac_conv_ref'] = summac_dict_ref['score_details_per_sample']['SummaC-Conv'][0]
    row['summac_conv_gen'] = summac_dict_gen['score_details_per_sample']['SummaC-Conv'][0]
...
metric_only_result_df = result_df.drop(columns=required_test_df_cols + ['output'])
